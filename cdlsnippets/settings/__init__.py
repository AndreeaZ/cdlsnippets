...
def deep_update(from_dict, to_dict):
    for (key, value) in from_dict.iteritems():
        if (key in to_dict.keys() and
                isinstance(to_dict[key], dict) and isinstance(value, dict)):
...
for module_name in modules:
    try:
        module = getattr(__import__(
            current, globals(), locals(), [module_name]), module_name)
    except ImportError, e:
        print 'ERROR: Unable to import %s configuration: %s' % (module_name, e)
        raise
    except AttributeError, e:
        if env == 'dev' and module_name == uid:
            print 'WARNING: Unable to import %s dev ' +\
                'configuration: does %s.py exist?' % (module_name, module_name)
        else:
            raise
